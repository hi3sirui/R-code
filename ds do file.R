...
##identifying empty surveys----
wave1 <- names(ds)[9:33]
wave2 <- names(ds)[37:64]
table(rowSums(is.na(ds[, wave1])))
table(rowSums(is.na(ds[, wave2])))

cat("Completely empty rows:", sum(rowSums(is.na(ds[, 9:ncol(ds)])) == (ncol(ds) - 8)), "\n")
cat("All Wave 1 NA:", sum(rowSums(is.na(ds[, wave1])) == 25), "\n")
cat("All Wave 2 NA:", sum(rowSums(is.na(ds[, wave2])) == 28), "\n")
both_empty <- rowSums(is.na(ds[, wave1])) == 25 & rowSums(is.na(ds[, wave2])) == 28
cat("Empty in both waves:", sum(both_empty), "\n")
